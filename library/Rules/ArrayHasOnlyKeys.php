...
#[Template(
    'Must have only the keys {{keys}} in {{name}}',
    'Must have not only the keys {{keys}} in {{name}}',
)]
final class ArrayHasOnlyKeys extends Standard
{
    ...
    public function evaluate(mixed $input): Result
    {
        ...
        if (count(array_diff(array_keys($input), $this->keys)) > 0) {
